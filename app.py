    ...
    with tab_gen:
        ...
        with col_arg:
            ...
            with st.spinner("Sincronizando datos locales..."):
                ...
                # ACTUALIZADO: Manejo de tendencias Merval
                merv_val = merv.get('valor') if merv else None
                merv_vd = merv.get('var_diaria') if merv else None
                merv_1m = merv.get('var_1m') if merv else None
                merv_6m = merv.get('var_6m') if merv else None
                ...
                html_caja = f"""<div style="background-color: #12161f; padding: 15px; border-radius: 8px; border: 1px solid #2a2e39; margin-bottom:15px; display: flex; justify-content: space-between;">
                    ...
                    <div style="width: 48%; border-left: 1px solid #2a2e39; padding-left: 15px;">
                        ...
                        <h3 style="margin:5px 0; color:#fff;">{f"{merv_val:,.0f}" if merv_val else 'N/D'} <span style="font-size:1rem; color:{'#2ecca6' if merv_vd and merv_vd > 0 else '#ff6b6b'};">({f"{merv_vd:.2f}%" if merv_vd else "-"})</span></h3>
                        {merv_tendencias}
                    </div>
                </div>"""
        ...
        with col_int:
            ...
            with st.spinner("Sincronizando contexto global..."):
                ...
                # ACTUALIZADO: Tabla expandida con historial
                ...
                for nombre, datos in macro_int_data.items():
                    ...
                    def formatear_celda(valor, suffix="%"):
                        if valor is None: return "<span style='color:#8ba1b6;'>-</span>"
                        if abs(valor) < 0.001: return "<span style='color:#8ba1b6;'>0.00</span>"
                        color = "#2ecca6" if valor > 0 else "#ff6b6b"
                        return f"<span style='color:{color}; font-weight:bold;'>{valor:+.2f}{suffix}</span>"
                    
                    suffix = " pts" if "%" in nombre else "%"
                    val_str = f"{val:.2f}" if val is not None else "N/D"
                    vd_str = formatear_celda(vd, suffix)
                    v1m_str = formatear_celda(v1m)
                    v6m_str = formatear_celda(v6m)
                    
                    html_int += f"<tr><td class='col-header' style='text-align: left;'>{nombre}</td><td>{val_str}</td><td>{vd_str}</td><td>{v1m_str}</td><td>{v6m_str}</td></tr>"
