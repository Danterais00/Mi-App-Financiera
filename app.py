...
st.write("Análisis exhaustivo: Ratios, Eficiencia, Crecimiento y Valuación.")

# 2. ENTRADA DE TICKERS
tickers_raw = st.text_input("Tickers (separados por coma):", "AAPL, MSFT, NVDA, GOOGL, AMZN, TSLA").upper()
    ...
    with st.spinner('Restaurando ROA y Quick Ratio...'):
        for ticker in lista_tickers:
            try:
                ...
                analisis_completo[ticker] = {"nombre": info.get('longName', ticker), "rev_g": rev_growth, "eps_g": eps_growth}
    ...
    if datos_fundamentales:
        ...
        if datos_revenue:
            ...
            chart_r = alt.Chart(df_plot_r).mark_line(point=True).encode(
                x=alt.X('variable', sort=None), y=alt.Y('value_b', title='Revenue ($B)'),
        ...
        for ticker, pts in ranking_puntos.items():
            ...
            if ticker in analisis_completo:
                if analisis_completo[ticker]["rev_g"] > 5: c_extra += 1
                if analisis_completo[ticker]["eps_g"] > 5: c_extra += 1
            puntuacion_final.append({"ticker": ticker, "score": pts + c_extra})

        top_3 = sorted(puntuacion_final, key=lambda x: x["score"], reverse=True)[:3]
        c_rec = st.columns(3)
