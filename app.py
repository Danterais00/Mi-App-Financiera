...
TOOLTIPS = {
    ...
    "Div Yield (%)": "Rendimiento por dividendo: el interest en efectivo que recibes anualmente por poseer la acción.",
...
st.info("⚠️ Filtro Elite Activo: Beta < 1.5 y Upside > 0% obligatorio para el TOP 5. PER > 10 para punto de calidad. (Volúmenes no afectan el ranking).")

# 2. ENTRADA DE TICKERS (Tip: Usa .BA para CEDEARs locales, ej: AAPL.BA)
...
if tickers_raw:
    ...
    with st.spinner('Sincronizando volúmenes de mercado y ratios financieros...'):
        for ticker in lista_tickers:
            try:
                ...
                fila_fun = {
                    ...
                    "Volen del Día": info.get('volume'),        # Captura de volumen
                    "Volumen Promedio": info.get('averageVolume'), # Captura de volumen promedio
    ...
    # --- FUNCIÓN MAESTRA DE FORMATEO (MONEDA O NÚMERO ABSOLUTO) ---
    ...
    if datos_fundamentales:
        ...
        # --- 1. VALUACIÓN (CON FILAS DE VOLUMEN INFORMATIVAS) ---
        st.write("### 1. Valuación y Datos de Empresa")
        df_val = df_total.loc[["Empresa", "Precio", "Fair Value (Target)", "Upside (%)", "Beta (Volatilidad)", "Volen del Día", "Volumen Promedio"]]
        ...
        for idx in df_val.index:
            h1 += f'<tr style="background-color: #f2f2f2;"><td style="font-weight:bold; border:1px solid #ddd; padding:8px;">{"Volumen del Día" if idx == "Volen del Día" else idx}</td>'
            for col in df_val.columns:
                ...
                elif idx in ["Volen del Día", "Volumen Promedio"]: val_show = fmt_num(val, es_moneda=False) # Formato limpio sin $
                else: val_show = str(val)
                h1 += f'<td style="{style}">{val_show}</td>'
            h1 += '</tr>'
        st.write(h1 + '</table>', unsafe_allow_html=True)

        # --- 2. COMPARATIVA FUNDAMENTAL AVANZADA (AISLADA DE VOLUMEN) ---
        st.divider()
        st.write("### 2. Comparativa Fundamental Avanzada")
        # FIX EXCLUSIÓN: Eliminamos las métricas de volumen para que no contaminen la Tabla 2
        df_fun = df_total.drop(["Precio", "Fair Value (Target)", "Upside (%)", "Beta (Volatilidad)", "Volen del Día", "Volumen Promedio"])
