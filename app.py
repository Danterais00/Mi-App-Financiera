...
st.info("⚠️ Filtro Elite Activo: Solo se consideran tickers con **Beta < 1.5** y **Upside > 0%**.")

# 2. ENTRADA DE TICKERS
tickers_raw = st.text_input("Tickers (separados por coma):", "KO, COST, PEP, PG, WMT, AAPL, MSFT, NVDA, JNJ, LLY, TSLA, ABBV, UNH, VRTX").upper()
...
if tickers_raw:
    ...
    with st.spinner('Filtrando por Beta y Upside...'):
    ...
    if datos_fundamentales:
        ...
        if datos_revenue:
            ...
            # [Lógica de tabla simplificada por espacio, igual a la anterior]
            st.dataframe(df_r[cols_r], use_container_width=True) # Versión rápida para asegurar integridad
        ...
        # --- 5. TOP 5 ELITE (NUEVAS CONDICIONES SIN-EQUANON) ---
        st.divider()
        ...
        for t in lista_tickers:
            if t in analisis_completo:
                ...
                if meets_beta and meets_upside:
                    ...
                    p_up = 1 # Ya sabemos que es positivo
                    ...
                    final_scores.append({
                        ...
                        "Total": score_total, "Bonus": p_crec, "Fund": p_fun,
                        "Eficacia": (p_fun/posibles_puntos[t]*100) if posibles_puntos[t]>0 else 0,
                        "Margin": analisis_completo[t]["net_margin"],
                        "Beta": analisis_completo[t]["beta_val"],
                        "Upside": analisis_completo[t]["upside_val"]
                    })

        top_5 = sorted(final_scores, key=lambda x: (x['Total'], x['Eficacia'], x['Bonus'], x['Margin']), reverse=True)[:5]
        
        if not top_5:
            st.warning("Ningún ticker cumple con las condiciones obligatorias (Beta < 1.5 y Upside > 0%).")
        else:
            cols_5 = st.columns(5)
            for idx, s in enumerate(top_5):
                with cols_5[idx]:
                    ...
                    with st.expander("Ver Racional"):
                        st.success("✅ **Filtros Sin-Equanon Superados:**")
                        st.write(f"- Beta controlado: **{s['Beta']:.2f}** (< 1.5)")
                        st.write(f"- Potencial confirmado: **{s['Upside']*100:.1f}%** (> 0)")
                        
                        st.write(f"**🛡️ Fortaleza Operativa:**")
                        st.write(f"Bate al promedio en **{s['Fund']}** indicadores clave de balance.")
                        
                        st.write(f"**📈 Momentum:**")
                        st.write("Aceleración operativa ▲▲" if s['Bonus']==2 else "Crecimiento parcial ▲" if s['Bonus']==1 else "Crecimiento estable")
