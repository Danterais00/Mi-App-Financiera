...
if tickers_input:
    ...
    datos_revenue = []
    datos_eps = []
    # Diccionario para guardar el análisis final de cada empresa
    analisis_completo = {}
    ...
    with st.spinner('Ejecutando algoritmo de selección...'):
        for ticker in lista_tickers:
            try:
                ...
                # Guardar métricas para la recomendación final
    ...
    if datos_fundamentales:
        ...
        # --- RENDER REVENUE Y EPS ---
        st.divider()
        if datos_revenue:
            st.write("### 2. Evolución de Ingresos y Beneficios")
        ...
        # Calculamos Score Final: Puntos Fundamentales (70%) + Crecimiento (30%)
        puntuacion_final = []
        for ticker, pts in ranking_puntos.items():
            ...
            puntuacion_final.append({
                "ticker": ticker,
                "puntos_fun": pts,
                "score_total": pts + crecimiento_extra,
                "datos": analisis_completo.get(ticker, {})
            })
        ...
        for i, rec in enumerate(top_3):
            with cols_rec[i]:
                ...
                st.write(f"**Empresa:** {rec['datos'].get('nombre', 'N/A')}")
                st.metric("Puntos Fundamentales", f"{rec['puntos_fun']}/7")
                
                # Justificación
                rev_g = rec['datos'].get('rev_growth', 0)
                eps_g = rec['datos'].get('eps_growth', 0)
                
                justificacion = f"Se recomienda **{rec['ticker']}** porque supera al promedio en {rec['puntos_fun']} indicadores clave. "
                if rev_g > 0: justificacion += f"Presenta un crecimiento de ingresos del **{rev_g:.1f}%**. "
                if eps_g > 0: justificacion += f"Su beneficio por acción (EPS) ha mejorado un **{eps_g:.1f}%**, demostrando alta eficiencia operativa."
                else: justificacion += "Aunque su rentabilidad trimestral es volátil, su estructura de balance es superior a la media."
                
                st.info(justificacion)

        # Ranking completo abajo
        with st.expander("Ver ranking completo de indicadores"):
            for item in sorted(puntuacion_final, key=lambda x: x["score_total"], reverse=True):
                st.write(f"**{item['ticker']}**: {item['puntos_fun']} celdas verdes + {item['score_total'] - item['puntos_fun']} puntos de crecimiento.")
                st.progress(item['score_total'] / 9)
else:
    st.info("Ingresa los tickers para obtener el Top 3 de recomendaciones.")
